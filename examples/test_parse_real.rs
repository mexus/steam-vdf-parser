use std::fs;
use steam_vdf_parser::{parse_text, parse_binary};

fn main() {
    // Test localconfig.vdf (text format)
    println!("=== Parsing localconfig.vdf (text format) ===");
    let localconfig = fs::read_to_string("/home/mexus/.local/share/Steam/userdata/127648749/config/localconfig.vdf");
    match localconfig {
        Ok(content) => {
            match parse_text(&content) {
                Ok(vdf) => {
                    println!("Success!");
